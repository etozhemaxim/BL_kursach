...
def run_calculations_in_processes(tasks, task_info, baza, powder_type, desc):
    ...
    with ProcessPoolExecutor(max_workers=4) as executor:  # 4 процесса одновременно
                    ...
                    if ballistics_result:
                        ...
                        if info['powder_type'] == "Свободная":
                            ...
                            for powder, fraction in zip(info['powders'], info['fractions']):
                                ...
                                powders_info.append((powder_idx, int(1/fraction)))
                        else:
                            powders_info = []
                            for powder, fraction in zip(info['powders'], info['fractions']):
...
def main():
    ...
    # ВОЗВРАЩАЕМ ВСЕ ТИПЫ ПОРОХОВ
    mixture_types = ["ПО", "ПС", "БО", "Свободная"]
    
    total_start_time = time.time()
    
    print("=== ПОЛНЫЙ РАСЧЕТ С ПРЕЖНИМИ ДИАПАЗОНАМИ И ВСЕМИ ПОРОХАМИ ===")
    print(f"Использование памяти в начале: {check_memory_usage():.1f} MB")
    print("Все расчеты выполняются в отдельных процессах для устранения утечек")
